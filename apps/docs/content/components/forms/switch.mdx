...
import { SwitchControlledExample, SwitchUncontrolledExample } from './examples/switch-example';
...
<Usage
  components={{
    usage: <SwitchUncontrolledExample />,
    wrapper: Wrapper,
  }}
  serializedCode={`<form
    onSubmit={(event) => {
...
:root {
  ...
  --ex-switch-width-lg: calc(var(--spacing) * 14);
